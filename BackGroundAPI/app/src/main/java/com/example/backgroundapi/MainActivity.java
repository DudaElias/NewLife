...
public class MainActivity extends AppCompatActivity {
    ...
    protected void atualizarView(){
        if (produtoList != null){
            for (AlimentosAPI produto: produtoList) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.iniciar_tarefa){
            if (isOnline()) {
                ...
                task.execute("https://raw.githubusercontent.com/raulfdm/taco-api/master/scripts/combineJsonData/taco-general.json");
            }
            else
    ...
    private class MyTask extends AsyncTask<String, String, String> {
        ...
        protected void onPostExecute(String s) {
            produtoList = AlimentosJsonParser.parseDados(s);
            atualizarView();
            progressBar.setVisibility(View.INVISIBLE);
        }
        @Override
